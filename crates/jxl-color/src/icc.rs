...
pub fn colour_encoding_to_icc(colour_encoding: &ColourEncoding) -> Result<Vec<u8>> {
    ...
    let wp = match &colour_encoding.white_point {
        jxl_image::WhitePoint::D65 => illuminant::D65_LCMS,
        jxl_image::WhitePoint::Custom(xy) => CIExyY { x: xy.x as f64 / 1e6, y: xy.y as f64 / 1e6, Y: 1.0 },
        jxl_image::WhitePoint::E => illuminant::E_LCMS,
        jxl_image::WhitePoint::Dci => illuminant::DCI_LCMS,
    };

    let primaries = match &colour_encoding.primaries {
        jxl_image::Primaries::Srgb => primaries::SRGB_64,
        ...
        jxl_image::Primaries::Bt2100 => primaries::BT2100_64,
        jxl_image::Primaries::P3 => primaries::P3_64,
    };
    let primaries = util::primaries_to_xyy(primaries, wp);

    let tf = match colour_encoding.tf {
        jxl_image::TransferFunction::Gamma(g) => ToneCurve::new(1e7 / g as f64),
        jxl_image::TransferFunction::Bt709 => ToneCurve::new_parametric(4, &[20.0 / 9.0, 1.0 / 1.099, 0.099 / 1.099, 1.0 / 4.5, 0.081])?,
        jxl_image::TransferFunction::Unknown => todo!(),
        jxl_image::TransferFunction::Linear => ToneCurve::new(1.0),
        jxl_image::TransferFunction::Srgb => ToneCurve::new_parametric(4, &[2.4, 1.0 / 1.055, 0.055 / 1.055, 1.0 / 12.92, 0.04045])?,
    ...
    let mut prof = lcms2::Profile::new_rgb(&wp, &primaries, &[&tf, &tf, &tf])?;
    prof.set_header_rendering_intent(match colour_encoding.rendering_intent {
